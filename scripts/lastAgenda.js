...
async function lastAgenda(app) {
    ...
    if (fileData.files.length > 2) {
        ...
        const lastNoteContent = await fs.readFileSync(`${app.vault.adapter.basePath}/${lastFile}`, 'utf8');
        const lastNoteLines = lastNoteContent.split('\n');
        const agendaIndex = lastNoteLines.findIndex(line => line.includes('# Agenda'));
        const nextHeaderIndex = lastNoteLines.findIndex((line, index) => index > agendaIndex && line.startsWith('#'));
        const lastNoteAgenda = lastNoteLines.slice(agendaIndex + 1, nextHeaderIndex).join('\n');
